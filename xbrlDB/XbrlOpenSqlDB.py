...
XBRLDBTABLES = {
                ...
                "resources", "relationship_set", "root", "relationship",
...
class XbrlSqlDatabaseConnection(SqlDbConnection):
    def verifyTables(self):
        dbTable = self.tablesInDB()
        missingTables = XBRLDBTABLES - dbTable
    ...
    def identifyPreexistingDocuments(self):
        ...
        if docUris:
            query = "SELECT document_id, document_url FROM document WHERE document_url IN ("
            length = len(docUris)
            i = 0
            for x in docUris:
                i += 1
                query = query + x + ","
                if (i % 10 == 0):
                    query = query[:-1] + ") or document_url IN ("
    ...
    def insertResources(self):
        ...
        table = self.getTable('resources', 'resource_id',
    ...
    def loadXbrlFromDB(self, loadDBsaveToFile, loadReportId):
        ...
        factsTbl = self.execute(_(
                               ...
                               "left join entity_identifier ei on ei.entity_identifier_id = f.entity_identifier_id order by f.fact_id")
                               .format(reportId))
        prevId = None
        factRows = []
        cntxTbl = {}
